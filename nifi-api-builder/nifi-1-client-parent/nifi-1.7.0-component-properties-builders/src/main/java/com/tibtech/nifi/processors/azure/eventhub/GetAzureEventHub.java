...
public final class GetAzureEventHub {
   ...
   */
  public static final String SERVICE_BUS_ENDPOINT_PROPERTY = "Service Bus Endpoint";
   ...
   * To support Namespaces in non-standard Host URIs ( not .servicebus.windows.net,  ie .servicebus.chinacloudapi.cn) select from the drop down acceptable options 
   */
  public final String getServiceBusEndpoint() {
    return properties.get(SERVICE_BUS_ENDPOINT_PROPERTY);
  }

  /**
   * To support Namespaces in non-standard Host URIs ( not .servicebus.windows.net,  ie .servicebus.chinacloudapi.cn) select from the drop down acceptable options 
   */
  public final GetAzureEventHub setServiceBusEndpoint(final String serviceBusEndpoint) {
    properties.put(SERVICE_BUS_ENDPOINT_PROPERTY, serviceBusEndpoint);
    return this;
  }
   ...
   * To support Namespaces in non-standard Host URIs ( not .servicebus.windows.net,  ie .servicebus.chinacloudapi.cn) select from the drop down acceptable options 
   */
  public final GetAzureEventHub removeServiceBusEndpoint() {
    properties.remove(SERVICE_BUS_ENDPOINT_PROPERTY);
    return this;
  }
   ...
   * The primary key of the Event Hub Shared Access Policy
   */
  public final GetAzureEventHub setSharedAccessPolicyPrimaryKey(
      final String sharedAccessPolicyPrimaryKey) {
   ...
   * The number of partitions that the Event Hub has. Only this number of partitions will be used, so it is important to ensure that if the number of partitions changes that this value be updated. Otherwise, some messages may not be consumed.
   */
  public final GetAzureEventHub setNumberOfEventHubPartitions(
      final String numberOfEventHubPartitions) {
   ...
   * A timestamp (ISO-8061 Instant) formatted as YYYY-MM-DDThhmmss.sssZ (2016-01-01T01:01:01.000Z) from which messages should have been enqueued in the EventHub to start reading from
   */
  public final GetAzureEventHub setEventHubMessageEnqueueTime(
      final String eventHubMessageEnqueueTime) {
   ...
   * The number of events that a receiver should fetch from an EventHubs partition before returning. Default(100)
   */
  public final GetAzureEventHub setPartitionRecivierFetchSize(
      final String partitionRecivierFetchSize) {
   ...
   * The amount of time a Partition Receiver should wait to receive the Fetch Size before returning. Default(60000)
   */
  public final GetAzureEventHub setPartitonReceiverTimeoutMillseconds(
      final String partitonReceiverTimeoutMillseconds) {
