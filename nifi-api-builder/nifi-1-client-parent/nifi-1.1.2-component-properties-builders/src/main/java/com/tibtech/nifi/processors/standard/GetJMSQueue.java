...
public final class GetJMSQueue {
   ...
   * Whether or not the JMS Message Properties should be copied to the FlowFile Attributes; if so, the attribute name will be jms.XXX, where XXX is the JMS Property name
   */
  public final GetJMSQueue setCopyJmsPropertiesToAttributes(final String copyJmsPropertiesToAttributes) {
  ...
  public static final Map<String, String> build(final Function<GetJMSQueue, GetJMSQueue> configurator) {
    return configurator.apply(new GetJMSQueue()).build();
  }

  public static final Map<String, String> build(@DelegatesTo(strategy = Closure.DELEGATE_ONLY, value = GetJMSQueue.class) final Closure<GetJMSQueue> closure) {
    return build(c -> {
