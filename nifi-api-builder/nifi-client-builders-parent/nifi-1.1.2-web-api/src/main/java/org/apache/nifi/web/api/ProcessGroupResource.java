...
public class ProcessGroupResource extends ApplicationResource
{
	 ...
	 * @param id
	 *            The id of the process group.
	 * @param requestProcessGroupEntity
	 ...
	 * @param id
	 *            The id of the process group to be removed.
	 * @return A processGroupEntity.
	 */
	@DELETE
	@Consumes(MediaType.WILDCARD)
			...
			@ApiParam(value = "The process group id.", required = true) @PathParam("id") final String id)
	{
		...
		return withWriteLock(serviceFacade, requestProcessGroupEntity, requestRevision, lookup ->
		{
			...
			// ensure write to this process group and all encapsulated
			// components including templates and controller services.
			// additionally, ensure
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("{id}/process-groups")
	@ApiOperation(value = "Gets all process groups", response = ProcessGroupsEntity.class, authorizations = {
			@Authorization(value = "Read - /process-groups/{uuid}", type = "") })
	@ApiResponses(value = {
			@ApiResponse(code = 400, message = "NiFi was unable to complete the request because it was invalid. The request should not be retried without modification."),
			@ApiResponse(code = 401, message = "Client could not be authenticated."),
	 ...
	 * @param groupId
	 *            The group id
	 * @param requestRemoteProcessGroupEntity
			...
			@ApiParam(value = "The remote process group configuration details.", required = true) final RemoteProcessGroupEntity requestRemoteProcessGroupEntity)
	{
		...
		}, null, remoteProcessGroupEntity ->
		{
			...
			{
				throw new IllegalArgumentException(
						"The specified remote process group URL is malformed: " + remoteProcessGroupDTO.getTargetUri());
			}
			...
			@ApiParam(value = "The connection configuration details.", required = true) final ConnectionEntity requestConnectionEntity)
	{
		...
		{
			throw new IllegalArgumentException(String.format(
					"Unrecognized destination type %s. Expected values are [%s]",
		...
		return withWriteLock(serviceFacade, requestConnectionEntity, lookup ->
		{
			...
			// explicitly handle RPGs differently as the connectable id can be
			// ambiguous if self referencing
	 ...
	 * instance that is instantiated cannot be referenced at a later time,
	 * therefore there is no corresponding URI. Instead the request URI is
	 * returned.
	 * <p>
	 ...
	 * @param groupId
	 *            The group id
	 * @param requestCopySnippetEntity
			...
			@ApiParam(value = "The copy snippet request.", required = true) CopySnippetRequestEntity requestCopySnippetEntity)
	{
		...
		return withWriteLock(serviceFacade, requestCopySnippetEntity, lookup ->
		{
			...
			// flag to only perform the restricted check once, atomic reference
			// so we can mark final and use in lambda
			...
			// consider each processor. note - this request will not create new
			// controller services so we do not need to check
			// for if there are not restricted controller services. it will
			// however, need to authorize the user has access
			// to any referenced services and this is done within
			// authorizeSnippetUsage above.
		...
		}, null, copySnippetRequestEntity ->
		{
			...
			final FlowEntity flowEntity = serviceFacade.copySnippet(groupId, copySnippetRequestEntity.getSnippetId(),
					copySnippetRequestEntity.getOriginX(), copySnippetRequestEntity.getOriginY(),
	 ...
	 * template instance that is instantiated cannot be referenced at a later
	 * time, therefore there is no corresponding URI. Instead the request URI is
	 * returned.
	 * <p>
	 * Alternatively, we could have performed a PUT request. However, PUT
	 * requests are supposed to be idempotent and this endpoint is certainly
	 ...
	 * @param groupId
	 *            The group id
	 * @param requestInstantiateTemplateRequestEntity
			...
			@ApiParam(value = "The instantiate template request.", required = true) InstantiateTemplateRequestEntity requestInstantiateTemplateRequestEntity)
	{
		...
		}, null, instantiateTemplateRequestEntity ->
		{
			...
			// prune response as necessary
			for (final ProcessGroupEntity childGroupEntity : flowSnippet.getProcessGroups())
			{
				childGroupEntity.getComponent().setContents(null);
			}
			...
			final String snippetId, final boolean authorizeTransitiveServices)
	{
		...
		// ensure read permission to every component in the snippet including
		// referenced services
	 ...
	 *            request to create the template
			...
			authorizeSnippetUsage(lookup, groupId, requestCreateTemplateRequestEntity.getSnippetId(), true);
				...
				createTemplateRequestEntity ->
				{
					...
					final TemplateDTO template = serviceFacade.createTemplate(createTemplateRequestEntity.getName(),
							createTemplateRequestEntity.getDescription(), createTemplateRequestEntity.getSnippetId(),
			...
			@FormDataParam("template") final InputStream in) throws InterruptedException
	{
		...
		try
		{
			final JAXBContext context = JAXBContext.newInstance(TemplateDTO.class);
			final Unmarshaller unmarshaller = context.createUnmarshaller();
			final JAXBElement<TemplateDTO> templateElement = unmarshaller.unmarshal(new StreamSource(in),
					TemplateDTO.class);
		...
		catch (final JAXBException jaxbe)
		{
			...
			final String responseXml = String.format(
		...
		catch (final IllegalArgumentException iae)
		{
			...
			final String responseXml = String.format("<errorResponse status=\"%s\" statusText=\"%s\"/>",
